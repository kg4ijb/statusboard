...
<div class="fieldcontain ${hasErrors(bean: shiftInstance, field: 'dutyOfficer', 'error')} ">
    <label for="dutyOfficer">
        <g:message code="shift.dutyOfficer.label" default="Duty Officer"/>

    </label>
    <g:select id="dutyOfficer" name="dutyOfficer.id" from="${com.statusboard.Staff.list()}" optionKey="id"
              value="${shiftInstance?.dutyOfficer?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>

<div class="fieldcontain ${hasErrors(bean: shiftInstance, field: 'stationCommander', 'error')} ">
    ...
    <g:select id="stationCommander" name="stationCommander.id" from="${com.statusboard.Staff.list()}" optionKey="id"
              value="${shiftInstance?.stationCommander?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>

<div class="fieldcontain ${hasErrors(bean: shiftInstance, field: 'notes', 'error')} ">
    <label for="notes">
        <g:message code="shift.notes.label" default="Notes"/>

    </label>
    <g:select id="notes" name="notes.id" from="${com.statusboard.Note.list()}" optionKey="id"
              value="${shiftInstance?.notes?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
